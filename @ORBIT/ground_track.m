...
function ground_track(lat,lon,opts,planet)
    ...
    if isfield(opts,'planet') && ~strcmpi(opts.planet,'none')
        planet = opts.planet;
    elseif (nargin < 4) || ~isfield(opts,'planet') || isempty(planet)
    ...
    if strcmpi(planet,'Earth Coastlines') && (~isfield(opts,'planet') || ~strcmpi(opts.planet,'none'))
    ...
    elseif ~strcmpi(planet,'Blank') && ~strcmpi(opts.planet,'none')
    ...
    hold on;
    plot(lon(1:(j(1)-1)),lat(1:(j(1)-1)),'Color',Color,'LineStyle', LineStyle,'LineWidth',LineWidth);
    for i = 1:(length(j)-1)
        plot(lon(j(i):(j(i+1)-1)),lat(j(i):(j(i+1)-1)),'Color',Color, 'LineStyle',LineStyle,'LineWidth',LineWidth,'HandleVisibility','off');
    end
